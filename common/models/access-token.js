 ...
 * @property {Number} settings.accessTokenIdLength Length of the base64-encoded string access
 *   token. Default value is 64. Increase the length for a more secure access token.
 *
 * @class AccessToken
...
module.exports = function (AccessToken) {
	 ...
	 * @options {Object} [options] Each option array is used to add additional keys to find an
	 *   `accessToken` for a `request`.
	 * @property {Array} [cookies] Array of cookie names.
	 * @property {Array} [headers] Array of header names.
	 * @property {Array} [params] Array of param names.
	 * @property {Boolean} [searchDefaultTokenKeys] Use the default search locations for Token in
	 *   request
	 * @property {Boolean} [bearerTokenBase64Encoded] Defaults to `true`. For `Bearer` token based
	 *   `Authorization` headers, decode the value from `Base64`. If set to `false`, the decoding
	 *   will be skipped and the token id will be the raw value parsed from the header.
	...
	AccessToken.prototype.validate = function (cb) {
		try {
			...
			var isValid = isEternalToken ? eternalTokensAllowed : elapsedSeconds < secondsToLive;
